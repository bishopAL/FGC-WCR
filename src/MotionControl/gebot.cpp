...
void CGebot::UpdateLegStatus(int legNum)
{
    ...
    }  //在检测到编号为 legNum 的机械腿处于 attach 状态时，递减该腿的连接时间或次数。如果连接时间或次数耗尽（attchTimes[legNum] < 0），系统会关闭自动控制模式，并退出当前函数。这可能用于防止机械腿长时间保持连接状态，确保系统定期检查或调整其动作。
    else
...
Matrix<float,4,3> CGebot::FnnStepModify()
{
        ...
        return mfLegCmdCompPos-mfShoulderPosCompensation;
        //结束   
